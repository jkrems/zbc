...
class TypeParam extends IType {
  ...
  create(args, variables) {
    if (args === undefined) {
      return this._autoCreate(variables);
    }
    throw new Error(`Cannot create ${this.name} from ${args}`);
  }
}
...
class BaseType extends IType {
  ...
  _autoCreate(variables) {
    variables = variables || new Map();
    const args = this.params.map(function(param) {
      return param._autoCreate(variables);
    });
  ...
  derivedType(params) {
    return new Type(this.name, params, this.id);
  }
}
...
class TypeInstance extends ITypeInstance {
  constructor(type, args) {
    ...
    if (type.params) {
      ...
      type.params.forEach(function(param, idx) {
        console.log(param);
        param.create(undefined, varLookup).merge(args[idx]);
      });
    }
