...
class Scope {
  ...
  has(id) {
    return this._known.has(id) || (this._parent && this._parent.has(id));
  }
...
function setDefaultScope(moduleScope) {
  return new NodeTypeVisitor.Pre({
    ...
    NamespaceDeclaration(node) {
      node.scope = new Scope(node.parent.scope);
      node.parent.scope.set(node.name, node);
    },
...
function trackScopes(ast, moduleScope) {
  moduleScope = moduleScope || new Scope(new Scope());
  ast.traverse(setDefaultScope(moduleScope));
  return ast;
}
