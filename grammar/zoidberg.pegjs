...
Import
  = UsingToken __ alias:(Identifier "=" __)? path:ImportPath __ extraction:("." ImportExtraction __)? ";" {
    return new ZB.Using(path, extraction ? extraction[1] : null);
  }

ImportPath
  = first:ImportPathSegment rest:("." ImportPathSegment)* {
    return buildList(first, rest, 1);
  }
...
ImportExtractionList
  = first:Identifier rest:( __ "," __ Identifier)* {
    return buildList(first, rest, 3);
  }

ImportExtraction
  = "{" __ extractions:ImportExtractionList __ "}" {
    return extractions;
  }
